...
dufs /srv/notes --allow-all --auth admin:secret
...
```bash
# Docker
docker run -d --name notes-dav -p 8080:5000 \
  -v /srv/notes:/data \
  sigoden/dufs /data --allow-all --auth admin:secret
```

Then in Settings → Sync choose **WebDAV server** and enter the URL, username,
and password. The password is verified before anything is saved and is stored
in your OS keychain (Keychain, Credential Manager, or Secret Service) — never
in the app's config files. Android has no keychain backend, so it falls back to
the app-private data directory.
